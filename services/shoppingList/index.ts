...
async function SetShoppingListIngredientQuantityService(
  ingredientId: number,
  ...
  `,
    {
      $quantityNeeded: toDbNumber(quantityNeeded),
      $quantityBuyed: toDbNumber(quantityBuyed),
      $ingredientId: ingredientId,
    },
  );
}

async function SetShoppingListProductQuantityService(
  productId: number,
  quantityBuyed: number,
  quantityNeeded: number,
) {
...
export async function SetShoppingListItemQuantityService(
  ...
  if (type === "ingredients")
    await SetShoppingListIngredientQuantityService(
      itemId,
      quantityBuyed,
      quantityNeeded,
    );
  else
    await SetShoppingListProductQuantityService(
      itemId,
      quantityBuyed,
      quantityNeeded,
    );
